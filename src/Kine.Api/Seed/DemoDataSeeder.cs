...
public static class DemoDataSeeder
{
    ...
    private const string PractitionerId = "kine-1";
        ...
        IReimbursementCaseStore reimbursementStore)
    {
        ...
        SeedScheduling(schedulingStore, patients);
        var invoices = SeedBilling(invoiceStore, patients);
        SeedClinical(seanceStore, patients);
    ...
    private static void SeedScheduling(ISchedulingStore store, IReadOnlyList<Patient> patients)
    {
        ...
        var slot0900 = NewSlot(store, today.AddHours(9), today.AddHours(9.5), isBooked: true);
        var slot0930 = NewSlot(store, today.AddHours(9.5), today.AddHours(10), isBooked: true);
        NewSlot(store, today.AddHours(10.25), today.AddHours(10.75), isBooked: false);
        var slot1045 = NewSlot(store, today.AddHours(10.75), today.AddHours(11.25), isBooked: true);
        var slot1400 = NewSlot(store, today.AddHours(14), today.AddHours(14.5), isBooked: true);
        NewSlot(store, today.AddHours(15), today.AddHours(15.5), isBooked: false);
        ...
        var slotSophie = NewSlot(store, today.AddDays(-1).AddHours(9), today.AddDays(-1).AddHours(9.5), isBooked: true);
        NewAppointment(store, sophie, slotSophie, AppointmentStatus.Completed);

        var slotMarc = NewSlot(store, today.AddDays(-2).AddHours(11), today.AddDays(-2).AddHours(11.5), isBooked: true);
        NewAppointment(store, marc, slotMarc, AppointmentStatus.Completed);

        var slotAmina = NewSlot(store, today.AddDays(-7).AddHours(15), today.AddDays(-7).AddHours(15.5), isBooked: true);
        NewAppointment(store, amina, slotAmina, AppointmentStatus.Completed);

        var slotLouis = NewSlot(store, today.AddDays(-51).AddHours(10), today.AddDays(-51).AddHours(10.5), isBooked: true);
    ...
    private static void SeedClinical(ISeanceStore store, IReadOnlyList<Patient> patients)
    {
    ...
    private static PractitionerSlot NewSlot(ISchedulingStore store, DateTime startAtUtc, DateTime endAtUtc, bool isBooked)
    {
        var now = DateTime.UtcNow;
