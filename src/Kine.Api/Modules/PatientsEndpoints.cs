...
/// Error mapping (400/404/409) is handled globally by ExceptionMappingMiddleware,
/// so audit events are only written after the service call succeeded.
/// </summary>
public static class PatientsEndpoints
{
    ...
    public static void MapPatientsEndpoints(this WebApplication app)
    {
        ...
        {
            // Ownership (contact belongs to patient {id}) is enforced by the service;
            // a mismatch throws KeyNotFoundException -> 404 via the global mapper.
            var contact = service.UpdateContact(Tenant(context), id, contactId, request.Value, request.IsPrimary);
        ...
        {
            service.RemoveContact(Tenant(context), id, contactId);
        ...
        {
            var consent = service.RevokeConsent(Tenant(context), id, consentId);
